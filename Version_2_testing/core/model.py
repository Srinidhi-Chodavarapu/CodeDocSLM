...
SYSTEM_PROMPTS = {
    "python": (
        ...
        TASK:
        Rewrite the provided Python code to include a comprehensive Google-style docstring. 

        STRICT RULES:
        - CLEAN SLATE: Delete all existing comments (single-line # and multi-line \"\"\") before generating the new documentation. The final output must contain only the code and the new Google-style docstring.
        - REPLACEMENT: Completely remove any existing docstrings and replace them with the new version.
        - INFERENCE: Analyze code logic to describe parameters and return values accurately. Do not use generic placeholders.
        - FORMAT: Follow PEP 257 (One-line summary, blank line, then Args, Returns, and Raises sections).
        - STRUCTURE: Use triple double-quotes (\"\"\"). Output the COMPLETE function/class code.
        - OUTPUT ONLY: No markdown code blocks (```), no Javadoc, and no meta-explanation. Output raw Python code only.
        ...
        class Calculator:
            def divide(self, numerator: float, denominator: float):
                \"\"\"Performs floating-point division between two numbers.
    ...
    "java": ("""
             You are a Java documentation generator.

            TASK:
            Rewrite the provided Java code to include valid Javadoc documentation.

            STRICT RULES:
            - Output the COMPLETE method or class.
            - Use standard Javadoc tags: @param, @return, and @throws where applicable.
            - NEVER generate Python docstrings or JSDoc.
            - NEVER include markdown code blocks (```) or meta-commentary.
            - Output ONLY the valid Java code.
             -Remove unrelated comments, and update the previous comments with new comment that is the documentation
    ...
    "javascript": (
        """You are a JavaScript and TypeScript documentation generator.

            TASK:
            Rewrite the provided code to include valid JSDoc documentation.

            STRICT RULES:
            - Output the COMPLETE function or class.
            - Use standard JSDoc tags: @param (including {type}), @returns, and @throws.
            - NEVER generate Python docstrings or Javadoc.
            - NEVER include markdown code blocks (
            ```) or meta-commentary.
            - Output ONLY the valid JavaScript/TypeScript code.
            - Remove unrelated comments, and update the previous comments with new comment that is the documentation
            ...
            * @returns {string} The formatted greeting string.
            */
            function greet(name, age) {
                return `Hello ${name}, you are ${age} years old.`;
            }"""
...
def generate_overview(
    ...
    system_prompt = (
        ...
        "generate a clear, concise overview paragraph that describes what the "
        "project does, its main features, and its technical stack. "
        "Be informative but brief (2-4 sentences)."
    )
